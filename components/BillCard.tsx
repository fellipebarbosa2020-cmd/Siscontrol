...
const BillCard: React.FC<BillCardProps> = ({ bill, isSelected, onSelect, onUnpay, onOpenPayForm, onOpenPostponeForm, onAttachFile, onEdit, onToggleRecurring }) => {
  ...
  const renderStatus = () => {
    if (isPaid) {
      if (!paymentDate) return <div className="flex items-center text-sm text-green-600 dark:text-green-400 font-semibold"><CheckCircleIcon className="w-4 h-4 mr-2"/> Pago</div>;
      ...
      if (paymentDiffDays <= 0) {
        return <div className="flex items-center text-sm text-green-600 dark:text-green-400 font-semibold"><CheckCircleIcon className="w-4 h-4 mr-2"/> Pago em dia</div>
      } else {
        return <div className="flex items-center text-sm text-yellow-600 dark:text-yellow-400 font-semibold"><ExclamationTriangleIcon className="w-4 h-4 mr-2"/> Pago com {paymentDiffDays} dia{paymentDiffDays > 1 ? 's' : ''} de atraso</div>
      }
    }
    if (postponedCount > 0) {
      return (
    ...
    if (isOverdue) {
       return <div className="flex items-center text-sm text-red-600 dark:text-red-400 font-semibold"><ExclamationTriangleIcon className="w-4 h-4 mr-2"/> Vencida há {Math.abs(daysDiff)} dia{Math.abs(daysDiff) > 1 ? 's' : ''}</div>
    }
    if (daysDiff === 0) {
      return <div className="flex items-center text-sm text-blue-600 dark:text-blue-400 font-semibold"><ClockIcon className="w-4 h-4 mr-2"/> Vence hoje</div>
    }
    return <div className="flex items-center text-sm text-blue-600 dark:text-blue-400 font-semibold"><ClockIcon className="w-4 h-4 mr-2"/> Vence em {daysDiff} dia{daysDiff > 1 ? 's' : ''}</div>
  };
  ...
  return (
    <div className={`
      ...
      border-l-8 ${getBorderColor()} ${isPaid ? 'opacity-70' : ''} flex flex-col h-full
    `}>
      <div className="p-4 flex-grow flex flex-col">
          <div className="flex justify-between items-start mb-3">
              <div className="flex items-start gap-2">
                  <input type="checkbox" checked={isSelected} onChange={() => onSelect(id)} className="h-6 w-6 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 cursor-pointer flex-shrink-0 mt-1" />
          ...
          <div className="text-sm text-gray-600 dark:text-gray-300 space-y-2 mb-4">
              <p className="flex items-center gap-2"><UserIcon className="w-4 h-4 text-gray-400"/> {beneficiary}</p>
              <p className="flex items-center gap-2">
                <CalendarIcon className="w-4 h-4 text-gray-400"/>
                {new Date(dueDate).toLocaleDateString('pt-BR')}
                <span className="text-xs text-gray-400 dark:text-gray-500 capitalize">({new Date(dueDate).toLocaleDateString('pt-BR', { weekday: 'long' })})</span>
              </p>
              <div className="flex flex-wrap gap-2">
                  <span className="flex items-center gap-1 bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-full text-xs"><TagIcon className="w-3 h-3"/>{category || 'N/A'}</span>
      ...
      <div className="bg-gray-50 dark:bg-gray-800/50 p-2 flex justify-between items-center rounded-b-xl border-t dark:border-gray-700">
        <div className="flex items-center gap-1">
          {isPaid ? (
             <button onClick={() => onUnpay(id)} className="px-3 py-1 text-xs font-semibold rounded-lg transition-colors duration-200 text-gray-500 hover:text-red-600 dark:text-gray-400 dark:hover:text-red-400">
                Desfazer
             </button>
          ) : (
            <>
              <button onClick={() => onEdit(bill)} aria-label="Editar conta" className="p-2 text-gray-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors duration-200"><PencilIcon className="w-5 h-5" /></button>
              <button onClick={() => onOpenPostponeForm(id)} aria-label="Postegar conta" className="p-2 text-gray-500 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-full transition-colors duration-200"><ClockIcon className="w-5 h-5" /></button>
      ...
      {isExpanded && (
        <div className="bg-gray-50 dark:bg-gray-800/50 p-4 border-t border-gray-200 dark:border-gray-700 space-y-4">
